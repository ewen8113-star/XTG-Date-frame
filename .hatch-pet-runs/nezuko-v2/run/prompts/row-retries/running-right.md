Create Codex pet row `running-right` for `nezuko`: exactly 8 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the exact chibi pixel-art Nezuko identity, pink eyes, bamboo muzzle, long black hair with orange tips, pink kimono, dark haori, bow, proportions, palette, and crisp pixel edges from the legacy atlas.. Preserve silhouette, face, palette, material, proportions, markings, and props.
