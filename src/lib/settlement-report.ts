...
<style>
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .summary { display: grid; grid-template-columns: repeat(4, 1fr); margin: 14px 0; border: 1px solid #d4d4d8; }
  ...
  th:nth-child(1) { width: 16%; } th:nth-child(2) { width: 21%; } th:nth-child(3) { width: 11%; } th:nth-child(4) { width: 42%; } th:nth-child(5) { width: 10%; }
...
<section class="summary"><div><span>判定记录</span><strong>${report.rows.length}</strong></div><div><span>计奖</span><strong>${rewardedCount}</strong></div><div><span>不计奖</span><strong>${excludedCount}</strong></div><div><span>待审核</span><strong>${pendingCount}</strong></div></section>
<table><thead><tr><th>奖励项</th><th>明细对象</th><th>判定</th><th>判定说明</th><th>金额</th></tr></thead><tbody>${reportRowsHtml(report.rows)}</tbody></table>
